...
use crate::models::{FeedRequest, FeedResponse, IngestEvent, ScoredPostsQuery, PostCandidate};
use crate::pipeline::CandidatePipeline;
use crate::{user_store, social_graph};
...
pub async fn get_feed(
    ...
    if should_update_heartbeat {
        ...
        tokio::spawn(async move {
            let _: Result<(), _> = redis_conn_bg.zadd(online_key, user_id, now + crate::params::USER_ONLINE_TIMEOUT_SECS).await;
        });
    }
    ...
    if let Ok(cached_json) = redis_conn.lpop::<_, Vec<String>>(&redis_key, NonZeroUsize::new(limit)).await {
    ...
    if feed.is_empty() {
        ...
        let (text_vec, visual_vec) = user_store::get_user_vectors(user_id, &mut redis_conn, &state.qdrant).await.unwrap_or((vec![0.0; 384], vec![0.0; 512]));
        let is_cold = user_store::is_cold_start(user_id, &mut redis_conn).await.unwrap_or(true);
        let (following, blocked) = social_graph::get_social_graph(user_id, &state.http_client, &mut redis_conn).await.unwrap_or((vec![], vec![]));

        // Bug 4 & Missing 1: Load seen, served, and action sequence
        let seen_ids: Vec<i64> = redis_conn.smembers::<_, Vec<String>>(format!("seen_posts:{}", user_id)).await
            .unwrap_or_default().iter().filter_map(|s| s.parse().ok()).collect();
        
        let served_ids: Vec<i64> = redis_conn.smembers::<_, Vec<String>>(format!("served_posts:{}", user_id)).await
            .unwrap_or_default().iter().filter_map(|s| s.parse().ok()).collect();

        let action_sequence = user_store::get_action_sequence(user_id, 50, &mut redis_conn).await.unwrap_or_default();
        ...
        let query = ScoredPostsQuery {
            ...
            seen_ids,
            served_ids,
            user_action_sequence: Some(crate::models::UserActionSequence { actions: action_sequence }),
            ..Default::default()
        };
    ...
    (StatusCode::OK, Json(FeedResponse {
        request_id,
        feed,
        latency_ms: latency,
    ...
    if payload.event_type == "new_post" {
        ...
        tokio::spawn(async move {
            ...
            if let Ok(serialized) = serde_json::to_string(&candidate) {
                ...
                let _: Result<(), _> = redis_conn.lpush(crate::params::GLOBAL_DISCOVERY_KEY, &serialized).await;
                let _: Result<(), _> = redis_conn.ltrim(crate::params::GLOBAL_DISCOVERY_KEY, 0, 4999).await;
                ...
                let online_key = "online_users";
                if let Ok(uids) = redis_conn.zrangebyscore::<_, _, _, Vec<i64>>(online_key, now, "+inf").await {
                    for chunk in uids.chunks(500) {
                        let mut pipe = redis::pipe();
                        for uid in chunk {
                            let user_feed_key = format!("feed:{}", uid);
                            pipe.lpush(&user_feed_key, &serialized).ltrim(&user_feed_key, 0, 99).ignore();
                        }
                        let _: Result<(), _> = pipe.query_async(&mut redis_conn).await;
                        tokio::task::yield_now().await; // Give other tasks a chance to use Redis
                    }
                }
                info!("Ingestion & Fan-out completed in {}ms", start.elapsed().as_millis());
            }
        });
